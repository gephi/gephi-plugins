...
# Ref: http://code.activestate.com/recipes/215912-union-find-data-structure/
# modified by Chayant Tantipathananandh
#
...
using find.'''
    ...
    def contains(self, object):
        return object in self.objects_to_num
	def same(self, i, j):
		return self.find(i)==self.find(j)
    def to_list(self):
		'''convert to list'''
		return self.objects_to_num.keys()
    def toLists(self):
		'''convert to lists'''
		comp = {}
		for c in self.to_list():
			l = self.find(c)
			if l not in comp: comp[l] = []
			comp[l].append(c);
		return comp
    def toSets(self):
		'''convert to sets'''
