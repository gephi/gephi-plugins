...
public class SemanticWebImporter implements FileImporter, LongTask {
    ...
    public final boolean execute(final ContainerLoader loader) {
        ...
        GraphModel model = currentGraphController.getModel(dataWorkspace);

        DraftFactory draftFactory = container.factory();
        for (Node currentNode : model.getGraph().getNodes()) {
            NodeDraft nodeDraft = draftFactory.newNodeDraft();
            String nodeDraftLabel = currentNode.getNodeData().getLabel();
            nodeDraft.setLabel(nodeDraftLabel);
            nodeDraft.setId(nodeDraftLabel);
        ...
        for (Edge currentEdge: model.getGraph().getEdges()) {
            ...
            edgeDraft.setLabel(currentEdge.getEdgeData().getLabel());

            String nodeDraftSourceLabel = currentEdge.getEdgeData().getSource().getLabel();
            NodeDraft nodeDraftSource = loader.getNode(nodeDraftSourceLabel);

            String nodeDraftTargetLabel = currentEdge.getEdgeData().getTarget().getLabel();
