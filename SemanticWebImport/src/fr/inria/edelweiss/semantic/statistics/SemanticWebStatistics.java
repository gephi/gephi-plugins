...
public class SemanticWebStatistics implements Statistics {
     ...
     *
     * @param gm The input.
     * @param am The output. One attribute or more contains the result of the statistics.
     */
    @Override
    ...
    /**
     * Convert the result returned by a select counting the types into a map.
     *
     * @param selectOnGraph Result obtained with a select request. The first column must contain the name of the type, the second column must contain the number
     * of occurrences found for this type.
    ...
    /**
     * Register for each node who are its fathers and children. Note that a node can have several children and/or father.
     *
     * @param selectOnGraph
     * @return
     */
    ...
    /**
     * Add the count for each node of typeModel, and add nodes if they are present in countTypes, but not in typeModel. It can happen that a type is known with
     * rdf:type (used to obtain countTypes), but that there is no known rdfs:subClassOf relationship with it (letting the typeModel unaware of this type).
     *
     * @param typeModel
     * @param countTypes
     */
...
/**
 * Compares by ascending count of successors, then by alphabetical order if there is counts are equal.
 *
