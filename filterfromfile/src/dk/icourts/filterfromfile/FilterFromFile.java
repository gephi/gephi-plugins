...
/**
 * Filter that allows to filter the nodes based on a list of labels provided
 * by the user, through Gephi's interface
...
//<editor-fold defaultstate="collapsed" desc="Variables">
    ...
    private HashSet<String> nodesToKeep = new HashSet<String>();
...
//</editor-fold>
    ...
    public boolean evaluate(Graph graph, Node node) {
        ...
        if ( !nodesToKeep.isEmpty() && 
                nodesToKeep.contains(node.getNodeData().getLabel()))
            //Keep node if node's label is in the list
            return true;
            ...
            for (Node v: graph.getNeighbors(node).toArray()){
                if (this.nodesToKeep.contains(v.getNodeData().getLabel()))
                    return true;
            }
    ...
    public FilterProperty[] getProperties() {
        ...
        try {
            //Define the properties and their getters and setters
             keepList = FilterProperty.createProperty(this, HashSet.class, 
    ...
    public HashSet<String> getNodesToKeep(){
        return this.nodesToKeep;
    }
    
    public void setNodesToKeep(HashSet<String> ntk){
