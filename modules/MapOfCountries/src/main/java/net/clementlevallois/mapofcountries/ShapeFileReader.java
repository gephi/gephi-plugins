...
public class ShapeFileReader {
    ...
    private final Map<String, String> subregionCodes = new HashMap();
    private final Map<String, String> districtCodes = new HashMap();
    ...
    public Graph read(Graph gr, String country, String subregion, String region, String district) throws IOException {
        ...
        while ((line = br.readLine()) != null) {
            ...
            } else if (!country.equals("World") & !countryName.equals(country)) {
                continue;
            }
            ...
            for (int i = 3; i < fields.length; i++) {
                if (fields[i].startsWith("node")) {
                    ...
                    node.setSize(0f);
                    node.setAttribute("background_map_node", true);
                    node.setAttribute("lat", Double.valueOf(lat));
                    node.setAttribute("lng", Double.valueOf(lng));
                    node.setFixed(true);
                    gr.addNode(node);
                }
                if (fields[i].startsWith("edge")) {
                    ...
                    edge.setAttribute("background_map_edge", true);
                    gr.addEdge(edge);
                }
            }
        }
