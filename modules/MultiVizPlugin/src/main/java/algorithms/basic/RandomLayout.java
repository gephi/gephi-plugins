...
public class RandomLayout {
    ...
    public RandomLayout(int layerDistance, HashMap<String, List<Node>> layers, String initialLayer, Node initialNode, boolean sortLayers) {
    ...
    public void start() {
        ...
        for (String currentLayer : layers.keySet()) {
            ...
            float layerSize = VizUtils.getLayerSize(layers, biggestNode, currentLayer);
            
            for (Node node : layers.get(currentLayer)) {
                double dx = initialNode.x() + node.size() + (float) (-layerSize / 2 + layerSize * new Random().nextDouble());
                double ly = (y + layerSize / 2) + (float) (-layerSize / 2 + layerSize * new Random().nextDouble()) + node.size();
                node.setX((float) dx);
                node.setY((float) ly);
