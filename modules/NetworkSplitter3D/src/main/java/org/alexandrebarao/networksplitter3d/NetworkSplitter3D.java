...
/**
 *
 * @author Alexandre Barão  (Network Splitter 3D)
 * 
 */
...
public class NetworkSplitter3D implements Layout {
        ...
        if ( graphModel != null  ) {
          Graph graph = graphModel.getGraphVisible();
            ...
            for ( int i = 0 ; i < nodes3D.size(); i++ ) {
                 Node3D p = (Node3D) nodes3D.get(i);
                 
                  // safe reset node coordinates to the previous state 
                 
                  Node detectedNode = searchNode(graph, p.getId());
    ...
    public LayoutProperty[] getProperties() {
     ...
     public double getDataLaboratoryValue(Node n, Column col) {
      ...
      public Node searchNode(Graph graph, int id) {
        Node n = null;
      ...
      public Node3D searchNode3D(int id) {
        ...
        if ( nodes3D != null ) {
                while ( nodeK < nodes3D.size()) {
                    ...
                    Node3D currentNode = (Node3D) nodes3D.get(nodeK);
