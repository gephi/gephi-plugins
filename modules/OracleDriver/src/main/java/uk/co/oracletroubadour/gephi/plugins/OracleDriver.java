...
public class OracleDriver implements SQLDriver
{
    ...
    * 
    * <p>This is the format produced by SQLUtils.getURL():- jdbc:${Prefix}://${Host}:${Port}/${DBName}</p>
    * 
    * <p>This is one of the formats that Oracle can use:- jdbc:oracle:thin:@//${Host}:${Port}/${ServiceName}</p>
    * 
    * <p>With prefix "oracle:thin:@" this URL is passed in:  <code>jdbc:oracle:thin:<b>@:</b>//192.168.100.158:1521/XE</code></p>
    * 
    *<p><u>In order to get the correct URL format this method needs to rewrite "@:" to "@"</u>.</p>
    * 
    * 
   ...
   */
    ...
    private void registerDriver() throws Exception 
    {
	    ...
	    try 
	    {
		    ...
		    if (null!=System.getenv(JDBC_PATH)  && !"".equals(System.getenv(JDBC_PATH)) ) 
		    {
			    logger.warning("Adding JDBC_PATH");
			    pathList.add( System.getenv(JDBC_PATH) );
		    }

		    // Add any Oracle JDBC files under Oracle Home 
		    if (null!= System.getenv(ORACLE_HOME) && !"".equals(System.getenv(ORACLE_HOME)) )
		    {
			    File oracleHome = new File ( System.getenv(ORACLE_HOME) ) ;  
			    
			    if(oracleHome.exists()
				    && oracleHome.isDirectory()
			      )
			    {
				    logger.warning("Oracle Home exists"); 
				    // InstantClient - JAR files are in $ORACLE_HOME
				    for ( File ojdbcFile : oracleHome.listFiles( filenameFilter ) )
				    {
					    logger.warning("Adding Instant Client JDBC file "+ojdbcFile.getCanonicalPath()); 
				    ...
				    {
					    logger.warning("Oracle Client JDBC library exists"); 
					    for ( File ojdbcFile : jdbcDirectory.listFiles( filenameFilter ) )
					    {
						    logger.warning("Adding Oracle Client JDBC file"+ojdbcFile.getCanonicalPath()); 
						    pathList.add(ojdbcFile.getCanonicalPath()); 
					    }
				    }
			    }
