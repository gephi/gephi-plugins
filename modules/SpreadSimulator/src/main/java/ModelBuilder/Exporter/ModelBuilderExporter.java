...
public class ModelBuilderExporter implements PluginGeneralActionsManipulator {
    ...
    public void execute() {
        ...
        var transitionEdges = List.of(graph.getEdges().toArray()).stream().filter(x -> x.getLabel().equals("Transition"));
        ...
        var transitionList = transitionEdges.map(transition -> {
            ...
            var sourceNodeState = source.getAttribute("Description") != null ?
                    ...
                    :
                    new NodeState(source.getAttribute("NodeState").toString());
            var destinationNodeState = destination.getAttribute("Description") != null ?
                    new NodeState(destination.getAttribute("NodeState").toString(), destination.getAttribute("Description").toString())
                    :
